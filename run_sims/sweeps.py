...
# def set_importation_rate_for_importation_only_campaign(simulation, value):
#     campaign_builder = partial(build_importation_only_campaign,value)
#     simulation.task.create_campaign_from_callback(campaign_builder)
#     return {"importations_per_year": value}
...
# For historical archetypes



# def set_population_size_and_importation_rate(simulation, value):
#     raise NotImplementedError("not implemented importation rate sweep yet - this requires changing campaign file")
#     population_size_in_thousands = value[0]
#     importation_rate = value[1]
#
...
#     # build config
#     set_ento(simulation.task.config, archetype=archetype)
#     # use lookup table to get appropriate x_Temporary_Larval_Habitat for given target_prevalence
#     log10_x_larval_habitat = lookup_x_larval_habitat_from_target_prevalence(target_rdt_prevalence)
#     simulation.task.config.parameters.x_Temporary_Larval_Habitat = 10**log10_x_larval_habitat
#
#     # add in importations based on population size and importation rate
#     number_of_imports_per_year = population_size_in_thousands * importations_per_year_per_thousand




def lookup_x_larval_habitat_from_target_prevalence(target_rdt_prevalence):
    df = pd.read_csv(os.path.join(manifest.additional_csv_folder, "prevalence_lookup.csv"))
    return df.loc[df["target_rdt_prevalence"] == target_rdt_prevalence, "log10_x_larval_habitat"].values[0]
